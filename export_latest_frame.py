#!/usr/bin/env python3
"""
export_latest_frame.py — Run: python export_latest_frame.py

Opis:
    Samodzielny skrypt, który co zadany interwał kopiuje
    najnowszy plik JPG z datasetu do wskazanego katalogu WWW.

    - Wyszukiwanie najnowszej klatki w strukturze YYYY/MM/DD/HH
    - Kopiowanie z zapisem atomowym (tmp + rename)
    - Domyślne ścieżki pobierane z src.settings (DATA_DIR)
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Kopiuj najnowszą klatkę do WWW")
    parser.add_argument(
        "--source-dir",
        type=Path,
        default=DATA_DIR,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Czas między kolejnymi kopiami w sekundach",
    )
...
def _find_latest_image(source_dir: Path) -> Optional[Path]:
    """Zwraca najnowszy plik JPG z datasetu albo None."""
        ...
        while True:
            ...
            if latest is None:
                if not reported_empty:
                    print("[INFO] Brak plików w dataset. Czekam na pierwszą klatkę...")
                    reported_empty = True
                time.sleep(interval)
                continue
