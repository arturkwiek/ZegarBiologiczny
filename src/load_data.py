...
try:
    from src.settings import ZB_DATA_DIR, ZB_LABELS_CSV
except ImportError:
    ZB_DATA_DIR = None
    ZB_LABELS_CSV = None
...
def load_labels(labels_path: str | Path) -> pd.DataFrame:
    ...
    """
    labels_path = ZB_LABELS_CSV or Path("labels.csv")
...
def check_files_exist(df: pd.DataFrame, data_root: str | Path) -> list[str]:
    ...
    """
    data_root = ZB_DATA_DIR or Path("dataset")
...
def get_default_paths(project_root: Path | None = None) -> tuple[Path, Path]:
    ...
    data_root = os.environ.get("ZB_DATA_DIR")
    labels_path = os.environ.get("ZB_LABELS_CSV")
