...
def time_based_split(
    ...
    """Podział danych w czasie na zbiory train/val/test.

    Założenia:
    - dane są sortowane rosnąco po kolumnie `datetime_col`
      (najstarsze rekordy trafią do train, najnowsze do test),
    - nie używamy losowego shuffle ani stratify,
    - `ratios` określa udział próbek w (train, val, test).

    Zwraca:
        df_train, df_val, df_test (w tej kolejności)
    """

    if datetime_col not in df.columns:
        raise KeyError(f"Brak kolumny '{datetime_col}' w dataframe")

    r_train, r_val, r_test = ratios
    total = r_train + r_val + r_test
    if not np.isclose(total, 1.0):
        # normalizujemy proporcje, aby uniknąć błędów zaokrągleń
        r_train, r_val, r_test = (r_train / total, r_val / total, r_test / total)

    # Bezpieczne parsowanie kolumny datetime (obsługa stringów).
    dt = pd.to_datetime(df[datetime_col], errors="coerce")
    if dt.isna().any():
        raise ValueError(
            f"Nie udało się sparsować wartości w kolumnie '{datetime_col}' do datetime."
        )

    df_sorted = df.copy()
    df_sorted[datetime_col] = dt
    df_sorted = df_sorted.sort_values(datetime_col).reset_index(drop=True)

    # Grupowanie po dniach, aby całe dni trafiały do jednego ze zbiorów,
    # co ogranicza przecieki informacji między sąsiadującymi próbkami.
    day_series = df_sorted[datetime_col].dt.date
    unique_days = sorted(pd.unique(day_series))

    n_days = len(unique_days)
    if n_days < 3:
        raise ValueError(
            f"Zbyt mało unikalnych dni ({n_days}) do podziału na train/val/test bez przecieku."
        )

    # Przydział liczby dni do poszczególnych zbiorów z zachowaniem kolejności.
    train_days = int(n_days * r_train)
    val_days = int(n_days * r_val)

    # Zapewniamy co najmniej po jednym dniu w każdym zbiorze.
    train_days = max(train_days, 1)
    val_days = max(val_days, 1)
    if train_days + val_days >= n_days:
        # W razie potrzeby zmniejszamy liczbę dni w walidacji,
        # tak aby zawsze został przynajmniej 1 dzień na test.
        val_days = max(1, n_days - train_days - 1)

    train_end = train_days
    val_end = train_end + val_days

    train_dates = unique_days[:train_end]
    val_dates = unique_days[train_end:val_end]
    test_dates = unique_days[val_end:]

    # Ostateczna asekuracja: jeśli mimo wszystko test byłby pusty,
    # przenosimy ostatni dzień z walidacji do testu.
    if not test_dates:
        if not val_dates:
            raise ValueError("Nie udało się wydzielić dni dla zbioru testowego.")
        test_dates = [val_dates[-1]]
        val_dates = val_dates[:-1]

    day_array = day_series.to_numpy()
    mask_train = np.isin(day_array, train_dates)
    mask_val = np.isin(day_array, val_dates)
    mask_test = np.isin(day_array, test_dates)

    df_train = df_sorted[mask_train].reset_index(drop=True)
    df_val = df_sorted[mask_val].reset_index(drop=True)
    df_test = df_sorted[mask_test].reset_index(drop=True)

    # sanity checks: rozłączność i pełne pokrycie
    assert not np.any(mask_train & mask_val)
    assert not np.any(mask_train & mask_test)
    assert not np.any(mask_val & mask_test)
    assert np.all(mask_train | mask_val | mask_test), "Nie wszystkie próbki trafiły do splitu"
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device) -> Tuple[float, float, float]:
    ...
    for Xb, yb_sc, yb_hour in loader:
        Xb = Xb.to(device)
        # Ujednolicenie z treningiem: normalizujemy [sin, cos] do długości 1
        preds = model(Xb)
        preds = preds / (preds.norm(dim=1, keepdim=True) + 1e-8)
        pred_sc = preds.cpu().numpy()
...
def main() -> None:
    ...
    # --- podział czasowy: train/val/test (70/15/15) wg kolumny 'datetime' ---
    log("Split czasowy: train/val/test = 70/15/15 wg kolumny 'datetime'")
    df_tr, df_va, df_te = time_based_split(df, datetime_col="datetime", ratios=(0.7, 0.15, 0.15))

    def _n_days(dfx: pd.DataFrame) -> int:
        return dfx["datetime"].dt.date.nunique()

    log(f"Days: train={_n_days(df_tr)}, val={_n_days(df_va)}, test={_n_days(df_te)}")
    log(f"Samples: train={len(df_tr)}, val={len(df_va)}, test={len(df_te)}")

    def _hour_hist(dfx: pd.DataFrame) -> np.ndarray:
        return np.bincount(dfx["hour"].astype(int).to_numpy(), minlength=24)

    log(f"Hour hist train: {_hour_hist(df_tr).tolist()}")
    log(f"Hour hist val:   {_hour_hist(df_va).tolist()}")
    log(f"Hour hist test:  {_hour_hist(df_te).tolist()}")

    # --- macierze cech i wektorów celu ---
    # Uwaga: normalizację wyliczamy WYŁĄCZNIE na zbiorze train (poniżej).
    X_tr = df_tr[feature_cols].astype(np.float32).to_numpy()
    X_va = df_va[feature_cols].astype(np.float32).to_numpy()
    X_te = df_te[feature_cols].astype(np.float32).to_numpy()

    yh_tr = df_tr["hour"].astype(np.float32).to_numpy()
    yh_va = df_va["hour"].astype(np.float32).to_numpy()
    yh_te = df_te["hour"].astype(np.float32).to_numpy()

    ysc_tr = hour_to_sin_cos(yh_tr)
    ysc_va = hour_to_sin_cos(yh_va)
    ysc_te = hour_to_sin_cos(yh_te)
